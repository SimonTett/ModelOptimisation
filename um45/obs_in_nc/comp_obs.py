...
def means(cube,name=None,start_time=None,end_time=None,mask=None,mask_name=None,mask_attribute=None,verbose=0,timeMean=True):
    """ 
...
output_data.extend(means(insw,name='insw',start_time=start_time,end_time=end_time,verbose=verbose,timeMean=timeMean))
# clear sky is optional..

try:
    rsrc=iris.load_cube(files,'toa_outgoing_shortwave_flux_assuming_clear_sky',callback=fix_missing_data ) ## clear rsr
    output_data.extend(means(rsrc,name='RSRC',start_time=start_time,end_time=end_time,verbose=verbose,timeMean=timeMean))
except iris.exceptions.ConstraintMismatchError:
    pass


try:
    olrc=iris.load_cube(files,'toa_outgoing_longwave_flux_assuming_clear_sky',callback=fix_missing_data ) ## clear sky OLR
    output_data.extend(means(olr,name='OLRC',start_time=start_time,end_time=end_time,verbose=verbose,timeMean=timeMean))
except iris.exceptions.ConstraintMismatchError:
    pass
...
output_data.extend(means(net_flux,name='netflux',start_time=start_time,end_time=end_time,verbose=verbose,timeMean=timeMean))
...
precip=iris.load_cube(files,'precipitation_flux' & constrain_60S,callback=fix_missing_data ) ## Precipitation north of 60S

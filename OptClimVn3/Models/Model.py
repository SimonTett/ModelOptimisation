...
class Model(ModelBaseClass, journal):
    ...
    def set_status(self, new_status: str, check_existing: bool = True) -> None:
        ...
        if check_existing and (self.status not in expected_status):
            raise ValueError(
                f"Expected current status {self.status}  to be one of " + " ".join(expected_status)+f" as changing to {new_status}")
    ...
    def submit_model(self, run_info: dict,
        ...
        Example:
         model.submit_model(config.run_info(),engine=SGE_engine,post_process_cmd = ['qrls', '890564.2'])
        """

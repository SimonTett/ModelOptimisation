...
def clear_logger_handlers(config: dict) -> None:
    """
    Clears all handlers for loggers referenced in a logging config dict.
    generated by chatGPT
    """
    # Clear handlers for named loggers
    for logger_name in config.get('loggers', {}):
        logger = logging.getLogger(logger_name)
        logger.handlers.clear()
        # handle sub loggers...
        for name, logger in logging.Logger.manager.loggerDict.items():
            if name.startswith(logger_name) and isinstance(logger, logging.Logger):
                logger.handlers.clear()
    # Clear handlers for root logger if present
    if 'root' in config:
        root_logger = logging.getLogger()
        root_logger.handlers.clear()

# Example usage:
# config_dict = {...}  # your dictConfig dictionary
...
def setup_logging(level:typing.Optional[typing.Union[int,str]] = None,
    ...
    if level is not None and log_config is not None:
        ...
        # Step 1 clear all existing loggers
        clear_logger_handlers(log_config)
        # step 2 set them up

...
class slurm_engine(abstractEngine):
    ...
    def submit_cmd(self, cmd: typing.List, name: str,
        ...
        if outdir is None:
            outdir = pathlib.Path.cwd() / 'output'
            my_logger.debug(f"Set outdir to {outdir}")
        submit_cmd = ['yhbatch', f'--mem={mem}', f'--mincpus={n_cores}', f'--time={time}',
        ...
        if isinstance(hold, list) and (len(hold) > 0):
            ...
            submit_cmd += [f'--dependency=afterok:' + ':'.join(hold)]#"--dependency=afterok:" + ":".join(hold) #liangwj
    ...
    def job_id(self, output: str) -> str:
        ...
        return output.split()[3].split('.')[0]#output.split()[2].split('.')[0] #liangwj
    ...
    def job_status(self, job_id: str, full_output: bool = False) -> str:
        ...
        if len(result) == 0:  # nothing found
            return "NotFound"
        status = result.split()[4]  #result.split(" ")[4] #liangwj
        if status.startswith("PENDING"):
            reason = result.split()[8].split("(")[1].replace(")", "")#reason = status.split("(")[1].replace(")", "") #liangwj
...
class slurm_engine(abstractEngine):
    ...
    def submit_cmd(self, cmd: typing.List, name: str,
        ...
        if outdir is None:
            outdir = pathlib.Path.cwd() / 'output'
            my_logger.debug(f"Set outdir to {outdir}")
        submit_cmd = ['yhbatch', '-n',f'1','-p',f'rhenv',#f'--mem={mem}', f'--mincpus={n_cores}', f'--time={time}',
                      '--output', f'{outdir}/{name}_%j.out', '--error', f'{outdir}/{name}_%j.err',
                      '-J', name]  #liangwj
        # --mem={mem}: Request mem mbytes  of memory per job
        # --mincpus={n_cores}: Request at least n_cores CPU per job
        # --time={time}: Request a maximum run time of time  minutes per job
    ...
    def job_status(self, job_id: str, full_output: bool = False) -> str:
        ...
        cmd = ["squeue", f"--job={job_id}", '--long']  # get the output in long form for specified job.
        if not full_output:
            cmd += ['--noheader']
        self.connect_fn(cmd)
        result = subprocess.check_output(cmd, text=True)

...
def ensure_clean_logger(logger_name, level=logging.DEBUG):
    """
    AI generated function to ensure a logger is clean and set to a specific level. This is useful for testing
    :param logger_name:
    :param level:
    :return:
    """
    ...
    print(f"Logger {logger_name} handlers after cleanup: {logger.handlers}")
class TestParamInfo(unittest.TestCase):
    ...
    def test_register(self):
        ...
        with self.assertLogs("OPTCLIM.param_info",level=logging.DEBUG) as log:
...
"""
    ...
    def test_update_from_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', prefix='.csv', delete=False) as tfile:
            ...
            self.expected_df.to_csv(file, index=False)
            tfile.close()
            with self.assertLogs('OPTCLIM.param_info',level=logging.WARNING) as log:
                p.update_from_file(file, duplicate=True)
            self.assertEqual(log.output, [

...
This version for UKESM. Tested with UKESM1.1 netcdf output
...
def do_work():
    ...
    Post process UKESM1 data to provide 32 simulated observations. Example use is:
    ...
    parser.add_argument("OUTPUT", nargs='?', default=None, type=pathlib.Path,
                        help="The name of the output file. Will override what is in the config file")
    parser.add_argument('input', nargs='+', default=[],type=pathlib.Path,
                        help='Input files. If not specified will be either specified in post_process options OR share/data/Hostory_data.nc')
    ...
    my_logger = genericLib.setup_logging(level=args.verbose)
    # setup processing

    config = StudyConfig.readConfig(genericLib.expand(args.CONFIG))
    options = config.getv('postProcess', {})
    if args.input: # got input so will use that.
        files=args.input
    else:
        globs= options.get('files',['share/data/History_Data/*.nc'])
        files  =[]
        for glob in globs:
            files += pathlib.Path.cwd().glob(glob)

    # mask_file,mask_name, start_time&  end_time  must all be provided.
    ...
    if args.OUTPUT:
        ...
        output_file = options.get('output_file','sim_obs.json')

    output_file = genericLib.expand(output_file)
    my_logger.info(f"mask_file: {mask_file}")
    my_logger.info(f"land_mask: {mask_name}")
    my_logger.info(f"start_time: {start_time}")
    my_logger.info(f"end_file: {end_time}")
    my_logger.info(f"output: {output_file}")
    my_logger.debug(f'files are \n')
    for file in files:
        my_logger.debug(f'file: {file}')
    my_logger.debug(f"options are:\n")
    ...
    latitude_coord = options.get('latitude_coord')
    ...
    for name, dataArray in process.items():
        ...
        my_logger.debug(f"Processed {name} and got {mean}")
    ...
    for name, value in results.items():
        my_logger.info(f"{name}: {value:.4g}")

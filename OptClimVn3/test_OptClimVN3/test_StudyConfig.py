...
class testStudyConfig(unittest.TestCase):
    ...
    def test_Covariances(self):
        ...
        cov = self.config.read_covariance(file=cov_file)
    ...
    def test_version(self) -> object:
        ...
        self.assertEqual(vn, 4, msg='Got vn = %d expected %d' % (vn, 4))
    ...
    def test_Fixed(self):
        ...
        # test that dict of param values works.
        c1 = fix
    ...
    def test_transMatrix(self):
        ...
        """
        if isinstance(self.config,StudyConfig.OptClimConfigVn4):
            print("WARNING: transMatrix no longer works. Not testing.")
            return
    ...
    def test_transform_matrix(self):
        ...
        for scale in [True, False]:
            ...
            inv_mat = self.config.transform_matrix(scale=scale, inverse=True)
            cov = self.config.Covariances(scale=scale)['CovTotal']
            got = tMat.dot(cov).dot(tMat.T)
            expect = np.identity(got.shape[0])  # trans matrix might, in effect, truncate matrix.
            atol = 1e-7
            ...
            with self.assertLogs(level='WARNING') as log:
                # Call the function that should log a warning
                trans = self.config.transform_matrix(scale=True)
            self.assertIn('WARNING:OPTCLIM.StudyConfig:Eigenvalues range is', log.output[1])

            # test that warning no longer gets generated by setting warn_scale to 1e-12. Should only have one log entry
            # complainign about something else!
            with self.assertLogs(level='WARNING') as log:
                # Call the function that should log a warning
                trans = self.config.transform_matrix(scale=True,warn_scale=1e-12)
            self.assertEqual(len(log.output),1)

            # test that increasing min_evale to 0.1 gives 1 ev.
            trans = self.config.transform_matrix(scale=True,min_evalue=0.1)
    ...
    def test_process_covariance(self):
        # test process_covariance works as expected --
        cov = self.config.Covariances(constraint=True)['CovTotal']
        index = ['olr_nhx', 'rsr_nhx', 'lat_nhx']
        # test scales work
        scales = pd.Series([2.0,2.0,2.0],index=index)
        got_cov = self.config.process_covariance(cov, scales=scales)
        expected_cov = cov.copy()
        for ind,v in scales.items():
            expected_cov.loc[ind,:] *= v
            expected_cov.loc[:,ind] *= v
        self.assertTrue(got_cov.equals(expected_cov))
        # test diagonalization works.
        expected_cov = pd.DataFrame(np.diag(np.diag(cov)), index=cov.index, columns=cov.index)
        got_cov = self.config.process_covariance(cov, diagonalize=True)
        self.assertTrue(got_cov.equals(expected_cov))
        # test obs_names
        obs_names = index
        expected_cov = cov.loc[index,index]
        got_cov = self.config.process_covariance(cov, obs_names=obs_names)
        self.assertTrue(got_cov.equals(expected_cov))

...
class Study:
    ...
    def read_model_configs(self, path_list: list[pathlib.Path],
                           parameters: typing.Optional[typing.List[str]] = None) -> typing.List[Model]:
        """
        ...
        :param parameters: If provided model parameters will be updated based on underlying model configuration.
        :return: models read in
        TODO: optionally specify parameters which get generated by reading the model config
        """
        models = []
        for f in path_list:
            try:
                logging.debug(f"Trying to load from {f}")
                m = Model.load_model(f,parameters=parameters)  # read the model.
